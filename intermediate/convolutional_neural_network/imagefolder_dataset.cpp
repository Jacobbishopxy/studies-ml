/**
 * @file:		imagefolder_dataset.cpp
 * @author:	Jacob Xie
 * @date:		2023/03/25 13:05:03 Saturday
 * @brief:
 **/
...
ImageFolderDataset::ImageFolderDataset(const std::string& root, ImageFolderDataset::Mode mode, torch::IntArrayRef image_load_size)
    : mode_(mode),
      image_load_size_(image_load_size.begin(), image_load_size.end()),
      mode_dir_(root + "/" + (mode == ImageFolderDataset::Mode::TRAIN ? "train" : "val")),
